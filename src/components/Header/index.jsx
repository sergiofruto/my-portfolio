const Header = ({ logo, bgColor, paddingTop }) => {
  const logoUrl = logo ? logo : "https://example.com/minimalist-icon.png";
  const backgroundColor = bgColor ? bgColor : "white";
  const textColor = bgColor ? "text-white" : "text-black";
  const paddingTopValue = `pt-[${paddingTop ? paddingTop : "0"}]`;

  const paddingTopClass = paddingTop ? `pt-[${paddingTop}]` : "";

  return (
    <header
      className={`${textColor} ${backgroundColor} ${paddingTopValue} body-font fixed w-full top-0`}
    >
      <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
        <a className="flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0">
          ...
          <span className="ml-3 text-xl text-white">The Brand</span>
        </a>
        <nav className="md:ml-auto flex flex-wrap items-center text-sm font-thin justify-center">
          <a className="mr-5 hover:text-gray-900">First Link</a>
          <a className="mr-5 hover:text-gray-900">Second Link</a>
          <a className="mr-5 hover:text-gray-900">Third Link</a>
          <a className="mr-5 hover:text-gray-900">Fourth Link</a>
        </nav>
        <button className="inline-flex items-center bg-orange-600 border-0 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base mt-4 md:mt-0">
          Button
          <svg
            fill="none"
            stroke="currentColor"
